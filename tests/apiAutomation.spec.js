...
test.beforeAll("Api Automation test", async () => {
    ...
    token = await APiUtil.createOrder(orderSet);
...
test("End to end test for e-commerse site", async({page})=> {
    ...
    const prodName =await page.locator("//tbody//tr").filter({ hasText: orderID }).locator('td').nth(1).textContent();
...
//     const dataSet = {client_id: "public_rest_api_client", grant_type: "password",username: "chanakaP", password: "0773379002Chanaka!"};

//     const apiContext = await request.newContext();
//     const response = await apiContext.post('https://orangehrm.orangehrm.com/oauth/issueToken',{
//     data:dataSet
//   })
